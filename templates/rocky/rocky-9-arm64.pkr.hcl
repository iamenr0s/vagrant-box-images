...
source "qemu" "rocky-9-arm64" {
  ...
  memory           = "${var.memory}"
  cpus             = "${var.cpu}"
  output_directory = "output/rocky/9/arm64"
  shutdown_command = "echo 'packer' | sudo -S shutdown -P now"
  ssh_password     = "${var.ssh_password}"
  ssh_port         = "${var.ssh_port}"
  ssh_timeout      = "${var.ssh_timeout}"
  ssh_username     = "${var.ssh_username}"
  vm_name          = "rocky-9-arm64"
...
build {
  sources = ["source.qemu.rocky-9-arm64"]
  ...
  post-processor "vagrant" {
    output = "output/rocky/9/arm64/rocky-9-arm64.box"
  }
}