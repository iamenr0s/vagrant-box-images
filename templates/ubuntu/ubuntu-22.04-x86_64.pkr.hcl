...
source "qemu" "ubuntu-22.04-x86_64" {
  accelerator      = "kvm"
  boot_command     = [
    "c<wait>",
  ...
  memory           = "${var.memory}"
  cpus             = "${var.cpu}"
  output_directory = "output/ubuntu/22.04/x86_64"
...
build {
  sources = ["source.qemu.ubuntu-22.04-x86_64"]
  ...
  post-processor "vagrant" {
    output = "output/ubuntu/22.04/x86_64/ubuntu-22.04-x86_64.box"
  }
}