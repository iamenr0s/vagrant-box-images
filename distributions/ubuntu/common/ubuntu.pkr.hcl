...
variable "version" {
  type    = string
  description = "Ubuntu version (20.04, 22.04, 24.04, etc.)"
}
...
// Ubuntu QEMU builder configuration
source "qemu" "ubuntu" {
  ...
  http_content = {
    "/user-data" = templatefile("${path.root}/distributions/ubuntu/common/http/user-data.pkrtpl.hcl", {
      version = var.version
    })
    "/meta-data" = templatefile("${path.root}/distributions/ubuntu/common/http/meta-data.pkrtpl.hcl", {
      version = var.version
    })
  }
}
